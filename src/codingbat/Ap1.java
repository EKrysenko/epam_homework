...
public class Ap1 {
    ...
    public String[] wordsWithout(String[] words, String target) {
        int i = 0;
        int j = 0;
        int count = countTargetOccurences(words, target);
        String[] wordsWithoutTargets = new String[count];
        while (i < words.length && j < count) {
            if (!words[i].equals(target)) {
                wordsWithoutTargets[j] = words[i];
                ++j;
            }
            ++i;
        }
        return wordsWithoutTargets;
    }
    private int countTargetOccurences(String[] words, String target) {
        int count = 0;
     ...
     * a walk beginning at the start index and ending at the end index, however increases in height count double.
